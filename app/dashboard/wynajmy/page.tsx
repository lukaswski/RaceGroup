...
const RENTAL_MOTORCYCLES = [
  {
    ...
    max: 20,
    price: 300,
    status: "gotowy" as RentalStatus,
    przeglad: "2025-03-20",
    ubezpieczenie: "2025-12-31",
  },
  {
    id: "r2",
    name: "Kawasaki Ninja 250",
  ...
  },
  {
    ...
    max: 20,
    price: 300,
    status: "zawieszony" as RentalStatus,
    przeglad: null,
    ubezpieczenie: null,
  },
  {
...
export default function WynajmyPage() {
  ...
  const confirmEdit = () => {
    ...
    if (!isNaN(price) && price >= 0) {
      setRentals((prev) => prev.map((r) => (r.id === editingId ? { ...r, price } : r)))
    }
    setEditingId(null)
    setEditingPrice("")
  }
      ...
      <table className="w-full min-w-[720px] table-fixed text-sm">
        ...
        <tbody>
          {rentals.map((m) => (
            <tr
              key={m.id}
              className="border-b border-border last:border-0 transition-colors even:bg-muted/40 hover:bg-muted/30"
            >
                  ...
                  {editingId === m.id ? (
                    <>
                      <input
                        ...
                        autoFocus
                      />
                      <span className="shrink-0 text-sm text-muted-foreground">zł</span>
                    </>
                  ) : (
        ...
        return (
          ...
          >
                    ...
                    {isEditing ? (
                      <span className="inline-flex items-center gap-1">
                        <input
                          ...
                          onChange={(e) => setEditingPrice(e.target.value)}
                          onKeyDown={(e) => e.key === "Enter" && confirmEdit()}
                          className="h-7 w-20 rounded-md border border-input bg-background px-2 text-xs outline-none focus:ring-2 focus:ring-ring"
                          autoFocus
                        />
            ...
            <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
              ...
              >
                Przegląd: {formatDate(m.przeglad ?? null)}
              </span>
              <span
                className={cn(
                  "rounded-lg px-2 py-0.5 font-medium",
              ...
              >
                Ubezp.: {formatDate(m.ubezpieczenie ?? null)}
              </span>
            </div>

            <div className="mt-3 flex flex-col gap-2 xs:flex-row xs:items-center xs:justify-end">
